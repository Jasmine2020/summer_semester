	...
	int nCmdShow)   // 用于指明窗口是否需要显示的参数。
{
	...
	mciSendString(TEXT("close 背景.wav"), 0, 0, 0);
	mciSendString(TEXT("play 背景.wav"), NULL, NULL, NULL);
...
void GamePaint(HWND hwnd)
{
	...
	*******************************************************************************/
	...
	// 创建需要用到的PEN和BRUSH
	hpen = CreatePen(0, 0, COLOR_BOUNDARY);  // PEN， PS_NULL表示不可见
